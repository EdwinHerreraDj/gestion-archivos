...
//Mostrar mensaje de eliminara carpeta
document.addEventListener('click', function (e) {
    if (e.target.classList.contains('delete-carpeta')) {
        e.preventDefault();
        const carpeta = e.target.getAttribute('data-carpeta');

        Swal.fire({
            title: '¿Estás seguro de eliminar esta carpeta?',
            text: "Perderás toda su información y no podrás recuperarla.",
            icon: 'warning',
            showCancelButton: true,
            ...
            customClass: {
                ...
                confirmButton: 'bg-red-500 text-white px-4 py-2 rounded hover:bg-red-600 focus:ring-2 focus:ring-red-300',
                cancelButton: 'bg-gray-500 text-white px-4 py-2 rounded hover:bg-gray-600 focus:ring-2 focus:ring-gray-300',
            },
            showCloseButton: false
        }).then((result) => {
            if (result.isConfirmed) {
                // Enviar el formulario para eliminar la carpeta
                document.getElementById('deleteForm-' + carpeta).submit();
            } else if (result.dismiss === Swal.DismissReason.cancel) {
                // Mostrar notificación de cancelación
                Swal.fire({
                    title: 'Acción Cancelada',
                    text: 'La carpeta ha sido conservada.',
                    icon: 'error',
                    confirmButtonClass: 'btn bg-primary text-white mt-2',
                    buttonsStyling: false
                });
